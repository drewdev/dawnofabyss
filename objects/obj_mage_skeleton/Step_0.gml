/// @description State machine logic for mage skeleton
...
if (knockback) {
    ...
    if (knockback_timer <= 0) {
        ...
        if (hp <= 0) {
            state = State.Dead;
            sprite_index = spr_skeleton_mage_dead;
            image_speed = 1;
            ...
            sprite_index = spr_skeleton_mage_walk;
            image_speed = 1;
    ...
    switch (state) {
        ...
        case State.Attack:
            sprite_index = spr_skeleton_mage_attack;
            if (hp <= 0) {
                state = State.Dead;
                sprite_index = spr_skeleton_mage_dead;
                image_speed = 1;
        ...
        case State.Hit:
            ...
            if (sprite_index == spr_skeleton_mage_hit && image_index >= sprite_get_number(sprite_index) - 1) {
                if (instance_exists(obj_player) && point_distance(x, y, obj_player.x, obj_player.y) < vision_range) {
                    state = State.Chase;
                    sprite_index = spr_skeleton_mage_walk;
                    image_speed = 1;
...
if (hp > 0 && invulnerability_timer <= 0 && place_meeting(x, y, obj_player) && obj_player.attacking) {
    ...
    state = State.Hit;
    sprite_index = spr_skeleton_mage_hit;
    image_speed = 1;
    image_index = 0; // Reset animation
